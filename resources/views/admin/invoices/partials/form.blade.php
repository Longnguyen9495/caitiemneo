                ...
                <fieldset class="border rounded-3 p-3 mb-2">
                    ...
                    <div class="row g-2">
                        <div class="col-12 col-lg-4">
                            ...
                            <select class="form-select mb-2" :id="`svc-${index}`" :name="`items[${index}][service_id]`"
                                    x-model="row.service_id" x-on:change="applyService(row)" @disabled(! $editable)>
                                <option value="">Dòng tự nhập</option>
                                {{-- Gom theo nhóm đúng như bảng giá giấy: 26 dịch vụ đổ
                                     thành một danh sách phẳng thì rất khó chọn trên điện thoại. --}}
                                    ...
                                    <optgroup label="{{ $groupLabel }}">
                                        @foreach ($grouped as $service)
                                            <option value="{{ $service->id }}"
                                                    ...
                                                    data-unit="{{ $service->unit?->value }}"
                                                    data-unit-label="{{ $service->quantityLabel() }}"
                                                    @if ($service->hasPriceRange())
                                                        data-min="{{ $service->price_min }}"
                                                        data-max="{{ $service->price_max }}"
                                                        data-range-label="{{ $service->formattedPriceRange() }}"
                                                    @endif
                                            >{{ $service->name }}</option>
                                        @endforeach
                                    </optgroup>
                            ...
                            <input class="form-control" type="text" :name="`items[${index}][name]`" x-model="row.name"
                                   placeholder="Tên hiển thị trên hóa đơn" required @disabled(! $editable)>
                        </div>

                        <div class="col-12 col-lg-3">
                            <label class="form-label" :for="`emp-${index}`">Nhân viên</label>
                            <select class="form-select" :id="`emp-${index}`" :name="`items[${index}][employee_id]`"
                                    x-model="row.employee_id" @disabled(! $editable)>
                        ...
                        <div class="col-6 col-lg-2">
                            ...
                            <input class="form-control text-end neo-num" :id="`qty-${index}`" type="number" step="0.01" min="0.01"
                                   :name="`items[${index}][quantity]`" x-model="row.quantity" required @disabled(! $editable)>
                        </div>

                        <div class="col-6 col-lg-3">
                            <label class="form-label" :for="`price-${index}`">Đơn giá</label>
                            <input class="form-control text-end neo-num" :id="`price-${index}`" type="number" step="1000" min="0"
                                   :class="priceOutOfRange(row) ? 'border-warning' : ''"
                                   :name="`items[${index}][unit_price]`" x-model="row.unit_price" required @disabled(! $editable)>
                            {{-- Khoảng giá là chỉ dẫn, không phải rào chặn: ca khó bất
                                 thường vẫn được chốt ngoài khoảng, chỉ cần thợ biết. --}}
                            <div class="form-text" x-show="row.range_label" x-cloak>
                        ...
                        <div class="col-6 col-lg-4">
                            ...
                            <select class="form-select" :id="`ctx-${index}`" :name="`items[${index}][work_context]`"
                                    x-model="row.work_context" @disabled(! $editable || ! $canApproveOvertime)>
                        ...
                        <div class="col-6 col-lg-3">
                            ...
                            <input class="form-control text-end neo-num" :id="`rate-${index}`" type="number" step="0.5" min="0" max="100"
                                   :name="`items[${index}][commission_rate]`" x-model="row.commission_rate"
                                   @disabled(! $editable || ! $canOverrideRate)>
                        </div>

                        @if ($canOverrideRate)
                            <div class="col-12 col-lg-5">
                                <label class="form-label" :for="`reason-${index}`">Lý do đổi tỷ lệ</label>
                                <input class="form-control" :id="`reason-${index}`" type="text"
                                       :name="`items[${index}][commission_rate_reason]`" x-model="row.commission_rate_reason"
                                       placeholder="Bắt buộc khi tự đặt tỷ lệ" @disabled(! $editable)>
                            </div>
                        @endif
                    </div>
            ...
            <div class="row g-3">
                <x-admin.field name="customer_name" label="Tên khách hàng">
                    <input class="form-control" id="customer_name" name="customer_name"
                           value="{{ old('customer_name', $invoice->customer_name) }}" @disabled(! $editable)>
                </x-admin.field>

                <x-admin.field name="customer_phone" label="Số điện thoại">
                    <input class="form-control neo-num" id="customer_phone" name="customer_phone" type="tel"
                           value="{{ old('customer_phone', $invoice->customer_phone) }}" @disabled(! $editable)>
                </x-admin.field>
